...
export const spectrogramTypes = {
  ...
  voronoiSpectrum: {
    ...
    points: [],
    numPoints: 20,
    pixelSize: 4, // Increased pixel size for better performance
    lastFrame: null,

    initialize: function(canvas) {
        // Pre-calculate static points array
        ...
        for(let i = 0; i < this.numPoints; i++) {
            this.points[i] = {
                ...
                y: 0,
                color: '',
                freqIndex: Math.floor(i * 1024 / this.numPoints) // Pre-calculate frequency indices
            };
        }
    ...
    draw: function(analyser, canvas, ctx, bufferLength, freqDataArray) {
        if (!this.offscreen) this.initialize(canvas);
        ...
        // Draw Voronoi cells with larger pixels
        for(let x = 0; x < canvas.width; x += this.pixelSize) {
            for(let y = 0; y < canvas.height; y += this.pixelSize) {
                ...
                for(let p of this.points) {
                    const dx = x - p.x;
                    const dy = y - p.y;
                    const dist = dx * dx + dy * dy; // Removed Math.hypot for performance
                    if(dist < minDist) {
                        minDist = dist;
                ...
                // Draw larger pixel
                this.offscreenCtx.fillStyle = closestColor;
                this.offscreenCtx.fillRect(x, y, this.pixelSize, this.pixelSize);
            }
        }
...
fractalSpectrogram: {
  ...
  draw: function(analyser, canvas, ctx, bufferLength, freqDataArray) {
      ...
      const drawBranch = (startX, startY, len, angle, depth, amplitude) => {
          if(depth === 0) return;
  ...
  draw: function(analyser, canvas, ctx, bufferLength, freqDataArray) {
      ...
      for(let i = 0; i < bufferLength; i++) {
          ...
          particles.forEach((p, index) => {
              ...
              ctx.beginPath();
              ctx.arc(p.x, p.y, amplitude * 15, 0, Math.PI * 2);
              ctx.fill();
  ...
  LayeredRippleVoronoi:{
    ...
    initialize: function(canvas) {
        ...
        for(let i = 0; i < this.numPoints; i++) {
            this.points[i] = {
                x: 0, y: 0, color: '',
                freqIndex: Math.floor(i * 1024 / this.numPoints)
            };
        }
    },
    
    draw: function(analyser, canvas, ctx, bufferLength, freqDataArray) {
        if (!this.points.length) this.initialize(canvas);
        ...
        for(let i = 0; i < this.numPoints; i++) {
            ...
            const angle = (i * 2 * Math.PI) / this.numPoints;
...
waterSpectrogram: {
    ...
    // Initialize once when first used
    initialize: function() {
      if (this.config.initialized) return;
      
      // Precalculate sine values
      for(let i = 0; i < 360; i++) {
        this.config.sinTable[i] = Math.sin(i * Math.PI / 180);
      }
    ...
    fastSin: function(x) {
      return this.config.sinTable[Math.floor((x % (Math.PI * 2)) * (180 / Math.PI)) % 360];
    },
    ...
    draw: function(analyser, canvas, ctx, bufferLength, freqDataArray) {
      ...
      // Initialize or resize points array if needed
      if (points.length !== layers) {
        points.length = 0;
        for(let z = 0; z < layers; z++) {
          points[z] = new Array(connections);
          for(let i = 0; i < connections; i++) {
            points[z][i] = {x: 0, y: 0, z: 0, perspective: 0};
          }
        }
      }
      ...
      for(let z = 0; z < layers; z++) {
        ...
        for(let i = 0; i < connections; i++) {
          ...
          point.y = canvas.height / 2 + 
                   ...
                   this.fastSin(currentTime + z/2 + i/10) * 20;
          point.z = zOffset;
    ...
    draw: function(analyser, canvas, ctx, bufferLength, freqDataArray) {
        ...
        for(let ring = 0; ring < 10; ring++) {
            ...
            for(let i = 0; i <= segments; i++) {
                ...
                // Add wave effect
                const waveOffset = this.fastSin(currentTime * 2 + ring + i/5) * 10;
